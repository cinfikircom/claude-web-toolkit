...
3. Framework değiştirmeyi önerme. Detay: `${CLAUDE_PLUGIN_ROOT}/skills/seo-geo-audit/references/code-safety.md`.
...
İşe başlamadan `.seo-os/seo-gorev-listesi.md`'yi **oku** (bağlamı devral). Durum işaretleriyle **güncelle**: `[~]`→`[✓]`; dış aksiyon gerekiyorsa `[!]` + ne beklendiği.

## Kapsam ve referanslar
- **Entity haritası & graf bağlama** → `${CLAUDE_PLUGIN_ROOT}/skills/seo-geo-audit/references/entity-graph.md`
...
- **Schema.org / JSON-LD** → `${CLAUDE_PLUGIN_ROOT}/skills/seo-geo-audit/references/schema-jsonld.md`
...
- **Knowledge conflict detector** → `${CLAUDE_PLUGIN_ROOT}/skills/seo-geo-audit/references/knowledge-conflict.md`
  — çapraz karşılaştırma matrisi + sabit "⚠ ENTITY CONFLICT DETECTED" raporu; sameAs Validation,
  Wikidata Mapping, Brand Consistency (`entity-graph.md` ile birlikte). NAP için `local-seo.md` ile koordine.
- **Citation için içerik yapısı** → `${CLAUDE_PLUGIN_ROOT}/skills/seo-geo-audit/references/geo-citation.md` (entity tanımları, FAQ).

## Çalışma akışı
1. `.seo-os/seo-gorev-listesi.md`'yi oku. Tüm siteyi tara; her sayfa tipi için varlıkları çıkar (tip + name + description + url + ilişkiler).
...
6. `.seo-os/seo-gorev-listesi.md`'yi güncelle + son rapora Entity/Knowledge Graph bölümünü ekle.
