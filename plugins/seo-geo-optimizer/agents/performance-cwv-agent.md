...
2. Render stratejisi (SSR→ISR/SSG) ve görsel/font pipeline değişiklikleri **High Risk** — geri alma planı ver.
   Anlık/kişisel veri içeren sayfaları ISR'ye çevirme. Detay: `${CLAUDE_PLUGIN_ROOT}/skills/seo-geo-audit/references/code-safety.md`.
...
- **CWV teşhis + raporlama formatı** → `${CLAUDE_PLUGIN_ROOT}/skills/seo-geo-audit/references/core-web-vitals.md`
  - INP derin teşhis (input delay / processing / presentation), DevTools/LoAF, `scheduler.yield()`, `content-visibility`.
- **Framework performans desenleri** → `${CLAUDE_PLUGIN_ROOT}/skills/seo-geo-audit/references/framework-performance.md`
  - Next.js/Astro/Nuxt/SvelteKit/HTML için görsel, font, render stratejisi, kod bölme.
- **Resource hints** → `${CLAUDE_PLUGIN_ROOT}/skills/seo-geo-audit/references/resource-hints.md`
  - preload/preconnect/dns-prefetch/prefetch/fetchpriority/modulepreload + Speculation Rules + Early Hints (103).
- **Lighthouse rubriği** → `${CLAUDE_PLUGIN_ROOT}/skills/seo-geo-audit/references/lighthouse-rubric.md`
  - Metrik ağırlıkları, mobile vs desktop, CrUX, puan→eylem matrisi.
- **Cloudflare edge** → `${CLAUDE_PLUGIN_ROOT}/skills/seo-geo-audit/references/cloudflare-edge.md`
  - CF arkasındaysa: Polish, Cloudflare Fonts, Early Hints, Cache Rules, R2/Workers; build'de minify.

## Çalışma akışı
1. `.seo-os/seo-gorev-listesi.md`'yi oku. Framework + render stratejisi + hosting/CDN (Cloudflare?) tespit et.
...
5. `.seo-os/seo-gorev-listesi.md`'yi güncelle + son rapora CWV bölümünü ekle; High Risk için geri alma planı.
